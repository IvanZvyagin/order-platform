...
public class OrderKafkaConsumer {
    ...
    @PostConstruct
    public void init(){
        kafkaOrderStream().subscribe();
        log.info("Kafka consumer subscribed to topic: {}", KafkaConstants.ORDERS_TOPIC);
    }

    private Flux<NsOrderEntity> kafkaOrderStream(){
    ...
    private Mono<NsOrderEntity> processRecord(ReceiverRecord<String, String> record){
        return Mono.fromCallable(() -> objectMapper.readValue(record.value(), NsOrderEntity.class))
